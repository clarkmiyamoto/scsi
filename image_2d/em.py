...
from si import loss_func, sample, loss_func_ell, sample_ell
from corruption import forward_channel, forward_channel_ell
...
def train_estep_ell(model: nn.Module, x_pool: torch.Tensor,
                    ell: float,
                    noise_std: float, p_drop: float, corruption: str, style: str,
                    epochs: int, batch_size: int, lr: float,
                    global_step: list,
                    ...
                    z_pool: torch.Tensor | None = None,
                    coupled_fraction: float = 0.0):
    '''E-step training conditioned on curriculum level ell.'''
    if z_pool is not None and coupled_fraction > 0.0:
        dataset = TensorDataset(x_pool, z_pool)
        has_z = True
    else:
    ...
    for epoch in range(1, epochs + 1):
        ...
        running = 0.0
        for batch in tqdm(loader,
                          desc=f"  E-step (ell={ell:.2f}) epoch {epoch}/{epochs}",
                          leave=False):
            if has_z:
                x_batch, z_batch_coupled = batch
                x_batch = x_batch.to(device)
            ...
            y_batch = forward_channel_ell(x_batch, ell=ell, noise_std=noise_std,
                                          p_drop=p_drop, corruption=corruption)

            loss = loss_func_ell(model, x_batch, y_batch, ell=ell, style=style, z=z_batch)
            opt.zero_grad(set_to_none=True)
            loss.backward()
            grad_norm = nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
...
@torch.no_grad()
def update_prior_ell(model: nn.Module, y_obs: torch.Tensor,
                     ell: float,
                     n_steps: int = 50, batch_size: int = 256, method: str = "euler",
                     device: torch.device = None) -> Tuple[torch.Tensor, torch.Tensor]:
    model.eval()
    ...
    z_chunks = []
    y_gpu = y_obs.to(device)
    for start in tqdm(range(0, N, batch_size), desc=f"  M-step (ell={ell:.2f})", leave=False):
        end = min(start + batch_size, N)
        initial_state = torch.randn(end - start, 1, IMAGE_SIZE, IMAGE_SIZE, device=y_gpu.device)
        x_batch = sample_ell(model, initial_state, y_gpu[start:end],
                              ell=ell, n_steps=n_steps, method=method)
        chunks.append(x_batch.cpu())
        z_chunks.append(initial_state.cpu())
...
def log_curriculum_step_wandb(x_gt: torch.Tensor, y_obs: torch.Tensor,
                               x_pool: torch.Tensor, ell: float, ell_idx: int, n: int = 8):
    y_slice = y_obs[:n, 0].cpu()
    vmin = float(y_slice.min())
    vmax = float(y_slice.max())
    if vmax - vmin < 1e-8:
    ...
    fig.suptitle(f"Curriculum level {ell_idx}  (ℓ={ell:.2f})", fontsize=12)
    plt.tight_layout()
    wandb.log({"curriculum/reconstruction": wandb.Image(fig),
               "curriculum/ell": ell}, step=ell_idx)
    plt.close(fig)