...
``dX/dt = b_t(X, t | y)`` from ``t=0`` (z') to ``t=1`` (x-hat). ``linear`` is the
optimal-transport interpolant; ``gvp`` is the geometric (variance-preserving) one.

Dimension-agnostic: operates on generic ``(..., D)`` point-cloud tensors, so this module is a
verbatim port of ``toy_3d_pc/si.py`` with ``D=2`` and a 1D sinogram conditioning signal
``(B, K, P)`` instead of a 2D tilt-series image ``(B, K, P, P)``.
"""
...
def interpolant(
    z: torch.Tensor,    # (B, N, 2) noise
    x: torch.Tensor,    # (B, N, 2) data / target clean cloud
    t: torch.Tensor,    # (B,) times in [0, 1]
    style: str = "linear",
) -> tuple[torch.Tensor, torch.Tensor]:
    """Return ``(I_t, dI_t/dt)`` for the chosen schedule."""
    tt = t.view(-1, *([1] * (x.dim() - 1)))           # (B, 1, 1) broadcast over (N, 2)
...
def transport_sample(
    model: nn.Module,
    z0: torch.Tensor,        # (B, N, 2) initial noise z'
    y: torch.Tensor,         # (B, K, P) conditioning sinogram
    n_steps: int = 50,
    integrator: str = "euler",
    eps_start: float = 0.0,
    eps_final: float = 0.0,
) -> torch.Tensor:
    """Integrate the conditional flow ODE from t=eps_start to t=1-eps_final -> x-hat (B, N, 2).
    ...
    context tokens on every step instead of re-running the signal encoder repeatedly.
