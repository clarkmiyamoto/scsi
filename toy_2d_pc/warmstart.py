"""Algorithm 1 -- warm-start / initialization of the drift b-hat^(0) for SCSI (CryoEM, 2D).
...
loop starts from a transport map that already roughly inverts ``F``:
    ...
    for i in 1 .. pretrain_steps:
        y ~ mu                      # a fixed observation (sinogram)
        g ~ SO(2)                   # random global pose (augments the unknown nuisance)
...
def find_initialization(
    model: nn.Module,
    y_obs: torch.Tensor,        # (Nobj, K, P) fixed observations -- CPU
    x_boot: torch.Tensor,       # (Nobj, N, 2) F-dagger(y_obs) -- CPU
    *,
    style: str = "linear",
