...
A point cloud is ``X in R^{N x 2}`` (a *set* of N points), so the network must be
...
The cloud is conditioned on the CryoEM observation ``y`` (a K-channel sinogram, i.e. a
K-tilt series of 1D signals) via cross-attention to its 1D patch tokens.
"""
...
class SignalPatchEncoder(nn.Module):
    """Encode a sinogram ``y`` (B, C, P) -> patch tokens for cross-attention.

    Signal patches carry positional meaning, so a learned positional embedding is added.
    ``in_channels`` = K stacks a CryoEM tilt series as conv input channels: the K
    1D projections fold into the patch embedding (token count -- and thus cross-attention
    cost -- is unchanged vs a single signal).
    """
    ...
    def forward(self, y: torch.Tensor) -> torch.Tensor:  # y: (B, C, P)
        h = self.proj(y)                       # (B, dim, P/ps)
        h = h.transpose(1, 2)                  # (B, n_patches, dim)
        return h + self.pos


class CrossAttnSetBlock(nn.Module):
    """Pre-norm: self-attn over points + cross-attn to signal tokens + per-point MLP."""
    ...
    def forward(self, h: torch.Tensor, ctx: torch.Tensor) -> torch.Tensor:
        # h: (B, N, dim) points; ctx: (B, M, dim) signal tokens
        x = self.norm1(h)
...
class ConditionalPointCloudVelocity(nn.Module):
    """b_t(X, t | y): (B, N, 2) x (B,) x (B, C, P) -> (B, N, 2), permutation-equivariant."""
    ...
    def __init__(
        ...
        self.sig_encoder = SignalPatchEncoder(image_size, patch_size, dim, in_channels)
    ...
    def encode_obs(self, y: torch.Tensor) -> torch.Tensor:
        """Encode observation y (B, C, P) -> ctx tokens (B, n_patches, dim).
        ...
        repeated Conv1d + positional-embedding work.
        """
        return self.sig_encoder(y)
...
class ConditionalModelConfig:
    ...
    heads: int = 4
    n_points: int = 256
    image_size: int = 64  # sinogram detector length P
    patch_size: int = 8
    in_channels: int = 11  # = n_tilts (K)
