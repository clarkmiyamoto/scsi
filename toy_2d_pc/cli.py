"""Command-line interface: ``python -m toy_2d_pc scsi ...``.

Runs lifted SCSI for a CryoEM-style 2D-image-to-1D-signal channel in the point-cloud
representation: recover a generative prior over clean 2D point clouds (MNIST-digit
silhouettes) from only their corrupted sinograms, via F-dagger bootstrap + warm-start + the
literal self-consistent EM loop. ``--supervised`` runs the oracle baseline instead. W&B
logging is ON by default (``--no-wandb`` to disable).
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="toy_2d_pc", description=__doc__)
    p.add_argument(
        "--device", default="auto", choices=["auto", "cuda", "mps", "cpu"],
        help="accelerator (default: auto-detect CUDA > MPS > CPU)",
    )
    sub = p.add_subparsers(dest="cmd", required=True)
    pe = sub.add_parser("scsi", help="lifted SCSI (CryoEM 2D->1D channel, point clouds)")
    ...
    pe.add_argument("--n-points", type=int, default=256, help="points per cloud (N)")
    pe.add_argument("--image-size", type=int, default=64, help="sinogram detector length P")
    pe.add_argument("--patch-size", type=int, default=8, help="signal-encoder patch size")

    # Forward model F.
    pe.add_argument("--radius", type=float, default=0.08, help="blob radius/sigma for G")
    pe.add_argument("--splat", choices=["gaussian", "disk", "histogram"], required=True,
                    help="G kernel: gaussian blob, filled disk, or histogram binning "
                         "(no default -- must be chosen explicitly)")
    pe.add_argument("--noise-std", type=float, default=0.1, help="Z: white signal-noise std")
    pe.add_argument("--coord-noise-std", type=float, default=0.0,
                    help="W: AWGN std on 2D point coords before rendering")
    ...
    pe.add_argument("--tomo-vol", type=int, default=64, help="F-dagger back-projection grid (vol^2)")
    ...
    pe.add_argument("--canonicalize", action="store_true",
                    help="canonicalize x-hat (align to a shared reference frame by ICP) before it enters "
                         "the interpolant target; no effect on F(x-hat) or the noise coupling")
    ...
    # Data / eval.
    pe.add_argument("--digit", type=int, default=0, choices=range(10), metavar="0-9",
                    help="MNIST digit class to draw the object pool from")
    pe.add_argument("--n-images", type=int, default=64,
                    help="size of the MNIST image pool (first N images of --digit, in dataset order)")
    pe.add_argument("--mnist-size", type=int, default=28,
                    help="resolution MNIST images are resized to before point-cloud sampling")
    pe.add_argument("--n-eval", type=int, default=6, help="objects shown in eval panels")
    pe.add_argument("--seed", type=int, default=0)
    pe.add_argument("--out", default="toy_2d_pc_checkpoint.pt", help="final checkpoint path")
    pe.add_argument("--eval-dir", default="toy_2d_pc_eval", help="where eval PNGs are written")
    ...
    pe.add_argument("--wandb-project", default="toy2d-pc-scsi")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.debug:
        ...
        args.n_objects, args.n_points = 8, 64
        ...
        args.n_images = min(args.n_images, 8)
        args.image_size, args.patch_size = 32, 8
        args.tomo_vol = 32
    ...
    with tracker:
        if args.supervised:
            ...
            train_supervised(
                ...
                digit=args.digit, n_images=args.n_images, mnist_size=args.mnist_size,
                ...
                n_tilts=args.n_tilts, tilt_step=args.tilt_step,
                splat=args.splat,
            )
        else:
            ...
            scsi_train(
                ...
                style=args.interpolant_style, n_eval=args.n_eval,
                ...
                tilt_step=args.tilt_step, splat=args.splat,
