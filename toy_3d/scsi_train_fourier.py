"""
toy_3d/scsi_train_fourier.py — SCSI for CryoEM 3D with Fourier-domain conditioning.

Key differences from scsi_train.py:
  - The velocity-field network is conditioned on the 2D DFT of y_obs (real + imaginary
    parts, normalized to [-1,1]) rather than raw pixel projections.  By the
    projection-slice theorem, FFT2D(sum_z x) = FFT3D(x)[kz=0, :, :], so this encoding
    directly gives the network the frequency content that the projection constrains —
    the same domain that real cryoEM reconstruction software works in.
  - A new "fourier" bootstrap: inserts the 2D DFT of each projection as the kz=0
    central slice of a 3D Fourier volume and IFFT3D back to real space (single-view
    Fourier backprojection via the central-slice theorem).
  - ConditionalUNet3DFourier uses in_channels=3: x_t + real(FFT2D(y)) + imag(FFT2D(y)).
  - Eval panels include a log|FFT2D(y_obs)| row showing the Fourier conditioning signal.
  - Fourier conditioning cond_obs = fourier_condition(y_obs) is precomputed once and
    reused every M-step (y_obs is fixed throughout SCSI).
...
def _mickey_mouse(grid_size, R_head, R_ear, ear_dx, ear_dy, cx, cy, cz, device="cpu"):
    """Mickey Mouse head: large sphere + two small spheres at top."""
...
# ── 2b. Fourier-domain utilities (NEW) ────────────────────────────────────────

def fourier_condition(y_obs: torch.Tensor) -> torch.Tensor:
    """
    Convert 2D projections to Fourier-domain conditioning tensors.

    By the projection-slice theorem, FFT2D(sum_z x)[ky, kx] = FFT3D(x)[kz=0, ky, kx].
    Conditioning the velocity field on this 2D DFT gives the network direct access to
    the frequency content that each projection constrains — the natural domain for
    cryoEM reconstruction.

    y_obs   : (B, 1, H, W) real-valued projections
    returns : (B, 2, H, W)  channels = [real, imag] of DC-centered FFT2D, normalized
              per-sample to [-1, 1] so the network sees a consistent input range.
    """
    Y   = torch.fft.fft2(y_obs[:, 0], norm="ortho")       # (B, H, W) complex
    Y_c = torch.fft.fftshift(Y, dim=(-2, -1))             # center DC component
    stack = torch.stack([Y_c.real, Y_c.imag], dim=1)      # (B, 2, H, W)
    # Per-sample normalization to [-1, 1]
    mx = stack.reshape(stack.size(0), -1).abs().max(dim=1).values
    mx = mx.view(stack.size(0), 1, 1, 1).clamp(min=1e-8)
    return stack / mx


def bootstrap_fourier_slices(y_obs: torch.Tensor, vol_size: int) -> torch.Tensor:
    """
    Fourier-slice bootstrap via the projection-slice theorem.

    Computes FFT2D(y_obs[i]) and inserts it as the kz=0 central slice of a 3D Fourier
    volume, then IFFT3D back to real space.  For the single-view (unknown-angle) case
    this is algebraically equivalent to tiling y_obs along the depth axis (since
    IFFT3D of a single-slice Fourier volume is z-invariant), but makes the Fourier
    relationship explicit and consistent with how multi-view cryoEM reconstruction
    combines slices in Fourier space.

    y_obs  : (N, 1, H, W)
    returns: (N, 1, D, H, W) in [-1, 1]
    """
    N, _, H, W = y_obs.shape
    D = vol_size
    device = y_obs.device

    Y   = torch.fft.fft2(y_obs[:, 0], norm="ortho")   # (N, H, W) complex
    Y_s = torch.fft.fftshift(Y, dim=(-2, -1))          # DC-centered

    # Crop or zero-pad to (D, D) when spatial resolution differs from vol_size
    if H != D or W != D:
        H2, W2 = min(H, D), min(W, D)
        new = torch.zeros(N, D, D, dtype=Y_s.dtype, device=device)
        sh, sw = (H - H2) // 2, (W - W2) // 2
        dh, dw = (D - H2) // 2, (D - W2) // 2
        new[:, dh:dh + H2, dw:dw + W2] = Y_s[:, sh:sh + H2, sw:sw + W2]
        Y_slice = new
    else:
        Y_slice = Y_s                                   # (N, D, D)

    # 3D Fourier volume: only the kz=0 central plane is populated
    F3D_s = torch.zeros(N, D, D, D, dtype=torch.complex64, device=device)
    F3D_s[:, D // 2, :, :] = Y_slice                   # kz=0 plane (DC-centered axis)

    # IFFT3D → real-space volume (take real part; imaginary is numerical noise)
    F3D = torch.fft.ifftshift(F3D_s, dim=(-3, -2, -1))
    x3d = torch.fft.ifftn(F3D, dim=(-3, -2, -1), norm="ortho").real.unsqueeze(1)

    # Normalize to [-1, 1] per sample
    flat = x3d.reshape(N, -1)
    mn   = flat.min(dim=1).values.view(N, 1, 1, 1, 1)
    mx   = flat.max(dim=1).values.view(N, 1, 1, 1, 1)
    return (x3d - mn) / (mx - mn + 1e-8) * 2.0 - 1.0


# ── 3. Conditional velocity-field model (Fourier conditioning) ────────────────

class ConditionalUNet3DFourier(nn.Module):
    """
    Velocity-field model conditioned on 2D Fourier features of projections.

    Input to UNet3D is the channel-concatenation of:
      ch 0   : x_t (current noisy volume)
      ch 1-2 : real and imaginary parts of DC-centered FFT2D(y_obs), each
               expanded along the depth axis to match the 3D volume shape.

    This differs from ConditionalUNet3D in scsi_train.py only in in_channels (3 vs 2)
    and the conditioning preparation.
    """
    def __init__(
        self,
        vol_size: int = 16,
        ...
        self.unet = UNet3DConditionModel(
            ...
            in_channels=3,           # x_t + real_fft + imag_fft
            out_channels=1,
    ...
    def forward(self, x_t, t, cond):
        ...
        cond : (B, 2, H, W)  real + imag channels of FFT2D(y_obs)
        """
        B, _, D, H, W = x_t.shape
        cond_3d = cond.unsqueeze(2).expand(-1, -1, D, -1, -1)  # (B, 2, D, H, W)
        inp = torch.cat([x_t, cond_3d], dim=1)                  # (B, 3, D, H, W)
        dummy = torch.zeros(
...
def si_loss_fourier(model, x, cond):
    """SI regression loss. cond: (B, 2, H, W) Fourier conditioning."""
    B  = x.size(0)
...
@torch.no_grad()
def sample_euler_fourier(model, cond, vol_size, n_steps=50):
    """Euler ODE integrator conditioned on Fourier features. cond: (B, 2, H, W)."""
    model.eval()
    B  = cond.size(0)
    x  = torch.randn(B, 1, vol_size, vol_size, vol_size, device=cond.device)
    dt = 1.0 / n_steps
    for i in range(n_steps):
        t_val = i * dt
        t_int = torch.full((B,), t_val * INTEGRATION_SCALE,
                           device=cond.device, dtype=torch.long)
        v = model(x, t_int, cond)
        x = x + v * dt
    return x
...
def train_estep_3d_fourier(
    ...
    """E-step: train velocity field on (x ~ π(k), fourier_condition(F(x))) pairs."""
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(loader, desc=f"    E-step epoch {epoch}/{epochs}", leave=False):
            ...
            # Generate a random projection, then convert to Fourier conditioning
            y_batch    = forward_channel(x_batch, noise_std)   # (B, 1, H, W)
            cond_batch = fourier_condition(y_batch)             # (B, 2, H, W)
            ...
            with torch.autocast(device.type, enabled=use_amp):
                ...
                I_dot_t = -z + x_batch
                t_int   = (t * INTEGRATION_SCALE).long()
                pred    = model(I_t, t_int, cond_batch)
...
@torch.no_grad()
def update_prior_3d_fourier(
    model: nn.Module,
    cond_obs: torch.Tensor,  # (N, 2, H, W) precomputed Fourier conditioning — CPU
    vol_size: int,
    n_steps: int,
    batch_size: int,
    device: torch.device,
    ...
    """
    Sample π(k+1) conditioned on the Fourier features of the fixed observations.

    cond_obs is precomputed once (= fourier_condition(y_obs)) and reused every M-step.
    Returns (x_pool, z_pool) on CPU.
    """
    model.eval()
    N        = cond_obs.size(0)
    x_chunks, z_chunks = [], []
    cond_gpu = cond_obs.to(device)
...
def _show_2d(ax, img2d: torch.Tensor, label: str | None = None, cmap="gray"):
    data = img2d.float().cpu().numpy() if isinstance(img2d, torch.Tensor) else img2d
...
def log_em_step_fourier(
    x_gt: torch.Tensor,      # (N, 1, D, H, W) ground-truth
    ...
    5-row panel:
      ...
      Row 1 — CryoEM observation y_obs        (2-D pixel space)
      Row 2 — log|FFT2D(y_obs)|               (Fourier conditioning signal)
      Row 3 — Pool sample x_pool[i]           (3-D render)
      Row 4 — F(x_pool[i])                    (2-D consistency check)
    """
    ...
    with torch.no_grad():
        ...
        Y      = torch.fft.fft2(y_obs[:n, 0], norm="ortho")          # (n, H, W) complex
        fft_mag = torch.log1p(torch.fft.fftshift(Y, dim=(-2, -1)).abs())  # (n, H, W)

    row_labels = [
        "GT (3-D)", "Obs y_obs", "log|FFT(y_obs)|",
        f"π({em_step}) sample", "F(pool)",
    ]
    ...
    for j in range(n):
        ...
        _show_2d(axes[2, j], fft_mag[j].cpu())
        axes[3, j].imshow(render_volume(x_pool[j, 0].cpu()), interpolation="nearest")
        axes[3, j].axis("off")
        _show_2d(axes[4, j], y_pool[j, 0])

    for r, label in enumerate(row_labels):
        axes[r, 0].set_ylabel(label, fontsize=8, rotation=0, labelpad=65, va="center")

    fig.suptitle(f"EM step {em_step} [Fourier conditioning]", fontsize=11, y=1.01)
    plt.tight_layout()

    if use_wandb:
        wandb.log({"em/panel": wandb.Image(fig)}, step=em_step)
    else:
        out = Path("toy3d_fourier_eval")
...
def make_rotation_movie(
    ...
    out_dir = Path("toy3d_fourier_eval")
    ...
    for i in tqdm(range(n_frames), desc="  rotation movie", leave=False):
        ...
        fig.suptitle(f"EM step {em_step} [Fourier]  |  z-rotation {deg:3d}°", fontsize=9)
        plt.tight_layout()
...
def parse_args():
    p = argparse.ArgumentParser(
        description="toy_3d SCSI-Fourier: unsupervised CryoEM 3D via EM with Fourier conditioning"
    )
    ...
    p.add_argument("--bootstrap",         type=str,   default="fourier",
                   choices=["tile", "noise", "revolve", "fourier"],
                   help=(
                       "π(0) initialisation: "
                       "'fourier' = projection-slice Fourier backprojection (default); "
                       "'tile' = tile y_obs along depth; "
                       "'revolve' = revolve y_obs around y-axis; "
...
if __name__ == "__main__":
    ...
    # Precompute Fourier conditioning once — reused every M-step
    print("Precomputing Fourier conditioning cond_obs = FFT2D(y_obs) ...")
    with torch.no_grad():
        cond_obs = fourier_condition(y_obs)    # (N, 2, H, W)  stays on CPU
    print(f"  cond_obs shape={tuple(cond_obs.shape)}"
          f"  range=[{cond_obs.min():.2f}, {cond_obs.max():.2f}]")
    ...
    if args.bootstrap == "fourier":
        x_pool = bootstrap_fourier_slices(y_obs, args.vol_size)
    elif args.bootstrap == "tile":
    ...
    model = ConditionalUNet3DFourier(
    ...
    if use_wandb:
        wandb.init(
            project="scsi-cryoem-toy3d-fourier",
            config=vars(args) | {"n_params": n_params, "N": N},
        )

    ckpt_dir = Path("toy3d_fourier_checkpoints")
    ...
    for k in range(args.n_em_steps):
        print("=" * 60)
        print(f"EM iteration {k} / {args.n_em_steps}  [Fourier conditioning]")
        print("=" * 60)

        epochs = args.epochs_first_pass if k == 0 else args.epochs_per_em

        # E-step: train velocity field on (x ~ π(k), fourier_condition(F(x))) pairs
        train_estep_3d_fourier(
            model, x_pool,
        ...
        # M-step: sample π(k+1) conditioned on precomputed Fourier features
        print(f"\n  M-step: sampling π({k+1}) ...")
        x_pool, z_pool = update_prior_3d_fourier(
            model, cond_obs,
            vol_size=args.vol_size,
