...
| `main.py` | CLI (`argparse`), device autodetect, `--debug` tiny-run defaults, dataset load, model/optimizer construction, optional `--init_ckpt` load, `wandb.init`/`finish`, one call to `em.run_em_loop(...)`. Also owns `--overfit` dispatch (see `overfit.py`) and `--warmup_steps` dispatch (see `warmup.py`) — the latter runs after `wandb.init`/`ckpt_dir` setup and before `run_em_loop`, feeding its returned step count into `run_em_loop`'s `global_step_start` so both phases share one continuous wandb run. |
| `warmup.py` | **`--warmup_steps` supervised warm-start, dispatched from `main.py` right before the EM loop — NOT part of the SCSI algorithm itself.** In-process analogue of `pretrain.py --warmstart_target classical_recon`, but reusing `main.py`'s own already-built `D_Y` (`y_obs`/`theta_star`/`acq_idx` from its `data.build_observations` call on the test split) instead of a separate train-split pretraining pool — no separate geometry flags, `main.py`'s own `--corruptions_per_object`/`--n_tilts`/`--tilt_increment_deg`/`--noise_std` size both this pool and the EM loop's pool at once, since they're the same `D_Y`. `backproject_pool(y_obs, theta_star, acq_idx, x_gt, recon_filter_type, recon_calibration, device=None)` — the per-acquisition `classical_recon.backproject` + affine-calibration loop, extracted from `pretrain.py::build_classical_recon_pool` so both files share ONE implementation; `pretrain.py` calls it (without `device`, since its pool is already GPU-resident) after its own `build_observations` call, `main.py`'s `run_classical_recon_warmup` calls it (with `device=device`, since `main.py` keeps `x_gt`/`y_obs`/`theta_star` CPU-resident — see `scsi.py::train_mstep`'s same per-batch `.to(device)` convention) directly against data it already has. `device`, when given, moves each acquisition's slice to `device` for the `backproject()` call and the result back to `CPU` before concatenating — the same compute-on-device/store-on-CPU pattern `scsi.py::propose_estep` already uses. `classical_target_for_display(...)` — moved from `pretrain.py`'s former `_classical_target_for_display`, now shared. `run_classical_recon_warmup(model, opt, x_gt, y_obs, theta_star, image_idx, acq_idx, args, device, use_wandb, ckpt_dir) -> int` — builds `x_recon_pool` once via `backproject_pool`, then flat supervised SGD on `scsi.py::loss_func_joint` over `(x_recon_pool, theta_star, y_obs)` triples (real `y`, never resynthesized — same choice `pretrain.py`'s classical-recon mode makes), logging via the new `wandb_logging.log_warmup_step` and periodically via the reused (caller-agnostic) `wandb_logging.log_pretrain_reconstruction`, checkpointing to `model_warmup.pt` (deliberately can't collide with `run_em_loop`'s `model_em{k:04d}.pt` glob). Returns the step count taken, which `main.py` passes straight into `em.run_em_loop`'s `global_step_start`. Composable with `--init_ckpt` (load a checkpoint, then further warm it up on this run's own `D_Y`) — intentional, not an error. |
...
# In-process classical-recon warmup on main.py's OWN D_Y, before the EM loop (see warmup.py)
uv run python main.py --debug --no_wandb --warmup_steps 4 --corruptions_per_object 2
uv run python main.py --warmup_steps 500 --corruptions_per_object 5   # keep corruptions_per_object
                                                                        # modest -- see gotcha below
# See sbatch/2d_cryoet_warmup/ for a full cluster matrix built on --warmup_steps: digit-coverage
# x corrupted-data-amount grid (--dataset_split train, --n_images_per_class up to 4096) plus
# --steps_per_em/--sample_steps sweeps, all sharing --warmup_steps 25000. Unlike
# sbatch/2d_cryoet/'s older two-script matrix (separate pretrain.py -> checkpoint -> main.py
# --init_ckpt jobs), every job in sbatch/2d_cryoet_warmup/ is a single main.py call.
...
`--dataset_split {train,test}` (default `test` — see the dataset-split gotcha below; switching
to `train` is only safe when there's no separate `pretrain.py` pool to stay disjoint from)
...
`--warmup_steps` (0 disables; see `warmup.py`'s row above) plus its
`--warmup_recon_filter_type`/`--warmup_recon_calibration`/`--warmup_checkpoint_every`/
`--warmup_plot_every` sub-flags.
...
- **`pretrain*.py` and `main*.py` draw from disjoint MNIST splits by default — `main.py` is
  the one exception, via the opt-in `--dataset_split` flag (see below).** All three `data.py`
  loaders (`load_mnist_subset`, `load_mnist_subset_mra`, `load_mnist_volumes_3d`) take
  `train: bool = True`; the three `pretrain*.py` scripts' `select_per_class` always pass
  `train=True` (60k-image train split, hardcoded, no flag); `main_3d.py`/`main_mra_rotation.py`
  still always pass `train=False` (10k-image test split, hardcoded); `main.py` passes
  `train=(args.dataset_split == "train")`, defaulting to `"test"` — same behavior as before the
  flag existed. WHY the guarantee originally existed as a hardcoded constant, not a flag: both
  pools used to draw unseeded random shuffles from the same hardcoded `train=True` split, so
  overlap between the pretraining pool and the EM dataset was possible and unrecorded even with
  non-overlapping `--digit_classes`. WHY `main.py` got the flag anyway: `--warmup_steps` (see
  `warmup.py`'s row above) builds its classical-recon pool from `main.py`'s OWN `D_Y` in-process
  — there is no separate `pretrain.py` pool for that guarantee to protect against overlapping
  with, and the test split's small per-class count (892–1135, see the CAVEAT below) couldn't fit
  `--n_images_per_class` much past ~1000 either way. `main.py --dataset_split` still defaults to
  `"test"`, so every OTHER caller (bare `main.py`, `--init_ckpt`-based runs) is unaffected.
  **CAUTION**: `--dataset_split train` combined with `--init_ckpt` pointing at a `pretrain.py`
  checkpoint (always train-split) DOES reopen the exact overlap hole this guarantee exists to
  close — that combination is on the caller, not guarded against in code. `main_3d.py`/
  `main_mra_rotation.py` did NOT get the equivalent flag (not requested, no in-process
  warmup-style caller for them yet) — they remain hardcoded `train=False`, no escape hatch.
  CAVEAT: the test split is much smaller per class (892–1135) than train (5421–6742);
  ...
  **silent short batch**, not an error — current defaults (2, up to 16 in `--debug`) are far
  below either cap, but e.g. `sbatch/2d_cryoet_warmup/`'s `--n_images_per_class` up to `4096`
  is exactly the case that needed `--dataset_split train` to avoid this. This does NOT make the
  draws reproducible (`seed` remains unused by every caller — it's threaded through every
  loader and `select_per_class` but no call site ever passes it). Scope: this guarantee is
  local to `mnist_cryoem/`; `image_2d/` and `toy_2d_pc/` load MNIST independently via their own
  `data.py` and are unaffected.
...
- **`main.py --warmup_steps` reuses the SAME `D_Y` the EM loop will train on — there is no
  separate warmup geometry.** `--corruptions_per_object`/`--n_tilts` size BOTH `warmup.py`'s
  classical-recon pool AND the EM loop's pool at once, since they're the same `y_obs`/
  `theta_star`/`acq_idx`. This means `warmup.py::backproject_pool` runs one
  `classical_recon.backproject()` call per acquisition (`n_images × corruptions_per_object`
  calls) once, up front, before the first warmup SGD step — at `main.py`'s own default
  (`--corruptions_per_object 200`), that's `n_images × 200` backproject calls, unlike
  `pretrain.py`'s own default of `1` for the same flag (see that file's row above). Keep
  `--corruptions_per_object` modest (e.g. single digits) when enabling `--warmup_steps` on a
  quick run; a full-scale run can afford the full default cost as a one-time setup cost, same as
  it already affords building the EM loop's own observation pool at that size. This differs from
  `pretrain.py`'s train/test-split disjointness guarantee (see the dataset-split gotcha above)
  by design — that guarantee exists to keep a *separate* pretraining pool from overlapping the
  EM dataset; here there is no separate pool, warmup and EM deliberately share one `D_Y`.
