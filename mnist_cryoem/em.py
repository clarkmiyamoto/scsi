...
from scsi import (
    ...
    propose_estep_3d, train_mstep_3d, train_mstep_mra,
)
from wandb_logging import (
    log_em_pool_diagnostics, log_reconstruction_grid,
    log_em_pool_diagnostics_3d, log_reconstruction_grid_3d, log_reconstruction_grid_mra,
)
...
########################################################
# Rotational-MRA outer EM loop. Mirrors run_em_loop, except there's no acq_idx/n_acq (no
# tilt-series grouping — see data.build_observations_mra), so a single `fixed_obs_id`
# (observation index) replaces `fixed_acq_id` (acquisition-group index). The E-step
# (propose_estep) and the pool health diagnostic (log_em_pool_diagnostics) are reused
# UNMODIFIED from the CryoEM pipeline — both are already channel-agnostic; only the per-column
# reconstruction panel (log_reconstruction_grid_mra) and the M-step (train_mstep_mra) are
# MRA-specific.
########################################################
...
def run_em_loop_mra(
    ...
    N_obs = y_obs.size(0)
    fixed_obs_id = 0  # tracked for the whole run so its reconstruction is comparable over time
    steps_first_em = args.steps_first_em if args.steps_first_em is not None else args.steps_per_em
    global_step = [0]
    ...
    for k in range(args.n_em_steps):
        ...
        # Visualization only, using this same Phi^(k-1) snapshot (before train_mstep_mra
        # updates the weights) so every diagnostic for iteration k reflects one consistent model.
        log_reconstruction_grid_mra(
            model, x_gt, y_obs, theta_star, image_idx,
            fixed_obs_id=fixed_obs_id, sample_steps=args.sample_steps,
            em_step=k, wandb_step=global_step[0], use_wandb=use_wandb, device=device,
        )
