...
def update_prior(
    ...
    print(
        ...
        f"std={x_pool.std():.3f}  surface_residual={torus_surface_residual(x_pool):.4f}"
    )
...
def log_em_step(
    ...
    plt.close(fig)
    resid = torus_surface_residual(pi_eval).item()
    print(f"  [eval] wrote {path}  pi surface_residual={resid:.4f} (torus target r^2=0.16)")
...
def scsi_train(
    ...
    print(f"[scsi] device={describe(device)}  amp={use_amp}")
    ...
    gt = sample_torus(n_objects, cfg.n_points, device=device)        # (Nobj, N, 3)
    with torch.no_grad():
    ...
    for k in range(em_steps):
        ...
        x_pool, z_pool = update_prior(
            model, y_obs, cfg.n_points, sample_steps, batch * 3, device
        )
        synchronize(device)
...
def train_supervised(
    ...
    for ``scsi_train``. If this learns a torus but SCSI doesn't, the fault is in the
    EM dynamics, not the model / channel / conditioning.
    """
    torch.manual_seed(seed)
    configure_backends(device)
    print(f"[supervised] device={describe(device)}  amp={use_amp}")
    ...
    gt_eval = sample_torus(n_eval, cfg.n_points, device=device)
    with torch.no_grad():
    ...
    model.train()
    for step in range(1, steps + 1):
        x1 = sample_torus(batch, cfg.n_points, device=device)   # fresh GT every step
        with torch.no_grad():
            y = forward_channel(
                x1, radius=radius, noise_std=noise_std,
                image_size=cfg.image_size, extent=extent,
