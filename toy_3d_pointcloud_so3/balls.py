...
We build a triangle mesh that is the union of one small **icosphere** per point,
write it as a Wavefront ``.obj`` (renders as a solid surface in the W&B 3D
viewer or any mesh tool), and can also rasterize a quick local PNG preview.
"""
...
def icosphere(subdivisions: int = 1) -> tuple[np.ndarray, np.ndarray]:
    """Unit sphere (radius 1, centered at origin) as a subdivided icosahedron.

    Args:
        subdivisions: 0 -> 12 verts / 20 faces (faceted); each level x4 faces.
    Returns:
        verts (V, 3) float32, faces (F, 3) int64 (0-indexed triangles).
    """
    t = (1.0 + 5.0 ** 0.5) / 2.0
    verts = [
...
def point_cloud_to_balls(
    ...
    Args:
        ...
        radius: ball radius (world units).
        subdivisions: icosphere resolution (see `icosphere`).
    Returns:
        verts (N*V, 3) float32, faces (N*F, 3) int64.
    """
    points = np.asarray(points, dtype=np.float32).reshape(-1, 3)
    sv, sf = icosphere(subdivisions)                       # (V,3), (F,3)
    n, v = points.shape[0], sv.shape[0]
    # Each ball: radius * unit_sphere + center.  Broadcast over N points.
    verts = (radius * sv[None] + points[:, None, :]).reshape(-1, 3)   # (N*V, 3)
    # Offset each ball's face indices by its vertex block start (i*V).
...
def render_balls_png(
    ...
    For speed each panel renders at most `max_balls` balls (the .obj keeps all
    of them). Poly3DCollection is slow, so keep `max_balls` modest.
    """
    import matplotlib
