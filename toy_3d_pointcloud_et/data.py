...
def sample_perturbed_dataset(
    ...
    """Dataset of ``n_objects`` bounded perturbations of fixed canonical template(s).

    Builds one canonical template cloud per shape in ``names`` (a fixed point
    configuration), then returns ``n_objects`` clouds, each a template (its shape
    drawn uniformly when ``len(names) > 1``) plus an independent per-point
    perturbation drawn uniformly from the ``perturb_eps``-ball, so every point moves
    by at most ``perturb_eps`` (``||delta_n|| <= perturb_eps``). With
    ``perturb_eps == 0`` all objects of a shape are identical copies of its template.

    This is the cryo-ET / subtomogram-averaging dataset: many noisy copies of the
    *same* underlying structure(s), each later imaged (under its own unknown pose) by
    the forward channel. Returns (n_objects, n_points, 3).
    """
    _check_shapes(names)
    ...
    if perturb_eps > 0:
        ...
        r = torch.rand(n_objects, n_points, 1, device=device).pow(1.0 / 3.0) * perturb_eps
        out = out + d * r                                      # uniform in the eps-ball
    return out
