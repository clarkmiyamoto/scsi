...
class ImagePatchEncoder(nn.Module):
    ...
    ``in_channels`` > 1 stacks a CryoET tilt series as conv input channels: the K
    projections fold into the patch embedding (token count is unchanged, so the
    cross-attention cost matches the single-image case).
    """
    ...
    def forward(self, y: torch.Tensor) -> torch.Tensor:  # y: (B, in_channels, P, P)
...
class ConditionalPointCloudVelocity(nn.Module):
    """v_theta(X, t | y): (B, N, 3) x (B,) x (B, C, P, P) -> (B, N, 3).

    Permutation-equivariant over the N points, conditioned on the observation y via
    cross-attention to its patch tokens. ``in_channels`` (C) is 1 for a single
    projection (so3/so2) or K for a CryoET tilt series stacked over channels.
    """
