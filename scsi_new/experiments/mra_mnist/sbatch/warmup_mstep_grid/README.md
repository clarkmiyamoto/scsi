...
(`--digit_classes 3`, `--n_images_per_class 6000`) under the MRA-style channel (unknown SO(2)
in-plane rotation + AWGN, no tilt series) in `experiments/mra_mnist/main.py`. Fixed everywhere
except the two swept axes: `--num_scsi_steps 40`.
...
all logged to the `scsi-mra-mnist-three-warmup-mstep-grid` wandb project so the whole grid is
filterable in one place. Job names are prefixed `mra_` (`mra_three_w20k_m10k`) so they're
distinguishable in `squeue` from the sibling grid in `cryoet_mnist/sbatch/warmup_mstep_grid/`,
which sweeps the same two axes under the cryoET-style tilt-series channel instead — this grid
answers "how much pretraining is needed" for the MRA channel.
