...
@torch.no_grad()
def log_reconstruction_grid(model, x0, theta, y, x_gt, noise_std, n_steps_sampling,
                            em_step, wandb_step, panel_name, device):
    x0, theta, y = x0.to(device), theta.to(device), y.to(device)
    x_hat = euler_integration(model, x0, y, n_steps_sampling)
    y_recorrupt = corruption_channel(x_hat, thetas=theta, noise_std=noise_std)
    ...
    for j in range(n):
        ...
        sino_lo, sino_hi = _pair_scale(y_c[j], y_recorrupt_c[j])
        axes[0, j].imshow(x_gt[j, 0].numpy(), cmap="gray", vmin=img_lo, vmax=img_hi)
        axes[1, j].imshow(y_c[j, :, 0, :].numpy(), cmap="gray", vmin=sino_lo, vmax=sino_hi, aspect="auto")
        axes[2, j].imshow(x_hat_c[j, 0].numpy(), cmap="gray", vmin=img_lo, vmax=img_hi)
        axes[3, j].imshow(y_recorrupt_c[j, :, 0, :].numpy(), cmap="gray", vmin=sino_lo, vmax=sino_hi, aspect="auto")
        for r in range(4):
...
@torch.no_grad()
def log_trajectory_grid(model, x0, theta, y, n_steps_sampling, n_snapshots, n_rows,
                        em_step, wandb_step, panel_name, device):
...
def random_draw(pool: dict, config_dataset, n: int) -> dict:
    ...
    theta = sample_tilt_series_angles(n, config_dataset.num_tilts, tilt_increment_rad)
    y = corruption_channel(x_gt, thetas=theta, noise_std=config_dataset.noise_std)
